...
func (s *SocketLayer) TlsPubKey() ([]byte, error) {
	...
	pubInterface := s.tlsConn.ConnectionState().PeerCertificates[0].PublicKey

	switch pub := pubInterface.(type) {
	case *rsa.PublicKey:
		return asn1ber.Marshal(*pub)
	case *ecdsa.PublicKey:
		// For ECDSA, you might want to marshal the X and Y coordinates
		return asn1ber.Marshal(struct {
			X, Y *big.Int
		}{pub.X, pub.Y})
	default:
